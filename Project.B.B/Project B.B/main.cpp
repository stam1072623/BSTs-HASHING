...
Node *insertNode(Node *node, int date, int volume) {
  ...
  if (volume < node->volumekey)
    node->left = insertNode(node->left, date, volume);
  else if (volume > node->volumekey)
  ...
  if (balanceFactor > 1) {
    if (volume < node->left->volumekey) {
      return rightRotate(node);
    } else if (volume > node->left->volumekey) {
      node->left = leftRotate(node->left);
      return rightRotate(node);
    }
  }
  if (balanceFactor < -1) {
    if (volume > node->right->volumekey) {
      return leftRotate(node);
    } else if (volume < node->right->volumekey) {
...
cout << "The AVL tree has been made. Please select one of the following options to continue with the program.\n";
    cout << "(1)->Display the AVL tree inOrder, (2)->Search for date with minimum volume, (3)->Search for date with maximum volume, (4)->Delete the record of a certain date, (5)->Exit the program.\n";
    ...
    while(loops)
    {
        ...
        case 2:
            ...
            cout << "You have selected option 2.The date with the minimum volume is:"<<findMin(root)<<endl;
            if(loops == 1)
            {
                cout << "Do you want to continue using the program?(Answer YES or NO) ";
                cin >> con;
        ...
        break;
        case 3:
            cout << "You selected option 3.The date with maximum volume is:"<<findMax(root)<<endl;
            if(loops == 1)
            {
                cout << "Do you want to continue using the program?(Answer YES or NO) ";
                cin >> con;
